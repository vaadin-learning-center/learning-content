...
:description: Learn why storing state on the server-side 
...
image::cluster.svg[Fig. 1 - A client makes a request to a 3-nodes cluster via a LB]
...
image::node-fails.svg[Fig. 2 - A client makes a request to a 2-nodes cluster via a LB]
...
image::non-sticky.svg[Fig. 3 - State stored on node A is not accessible if LB forwards client to node B on a subsequent request]
