...
NOTE: If your paper is wide, the text fields will line up horizontally. You can make the paper narrower by dragging its left or right edge. When the paper is narrow enough, the form fields will stack vertically.
...
public class MainView extends PolymerTemplate<MainView.MainViewModel> {
    ...
    public MainView(ContactService contactService) {
        ...
        filterText.setValueChangeMode(ValueChangeMode.LAZY); <2>
        filterText.addValueChangeListener(e -> updateList()); <3>
    }

    private void updateList() {
        grid.setItems(contactService.findAll(filterText.getValue())); <4>
    }
...
<1> Extract the call to grid.setItems into a new method
<2> Put text field value changes into lazy mode, so that we don't query the database on each keystroke
<3> Add value change listener to text field that tells the grid items to be updated
<4> Pass the value of the text field to the service findAll method. This will not compile yet as the method does not exist.
