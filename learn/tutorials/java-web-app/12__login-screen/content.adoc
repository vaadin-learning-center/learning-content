...
+
<1> `SecurityUtils`: Utility methods.
<2> `CustomRequestCache`: A cache to keep track of unauthenticated requests.
<3> `SecurityConfiguration`: Spring Security configuration.
+
