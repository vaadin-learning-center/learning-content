 ...
 * image:step-over.png[step over icon] *Step over*: Continue to the next line in the same file.

 * image:step-into.png[step into icon] *Step into*: Drill into a method call (for instance, if you wanted to see what's going on inside service.greet()).

 * image:step-out.png[step out icon] *Step out*: Go back to the line of code that called the method you're currently in.
+
