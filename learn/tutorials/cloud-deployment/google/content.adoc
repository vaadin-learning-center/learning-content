...
We use the Vaadin link:https://vaadin.com/start/latest[Spring Boot starter app] in our example. Please note that Vaadin Flow applications are stateful and require Session Affinity, or "sticky sessions". While App Engine supports Session Affinity, it's not recommended to use it for anything else than demo deployments link:https://cloud.google.com/appengine/docs/flexible/java/using-websockets-and-session-affinity[due to periodical server restarts]. *Option 2* shows how to deploy the app in a Docker container running in a Kubernetes cluster with Session Affinity enabled. Please note that this is only an example configuration for the demo app in question. You can read more about configuring CKE and deploying stateful applications in the link:https://cloud.google.com/kubernetes-engine/docs/how-to/stateful-apps[Google Cloud documentation].
