...
This tutorial shows how to deploy a Java web application to the Heroku cloud. Heroku provides free cloud deployment for up to 5 applications. Keep in mind that due to their automatic dyno restarts, you'll have to design your application to re-build the UI from url parameters or other external mechanisims. However, this platform is good for applications such as dashboards or demos. You can find more information about this on their https://devcenter.heroku.com/articles/dynos#automatic-dyno-restarts[website].
