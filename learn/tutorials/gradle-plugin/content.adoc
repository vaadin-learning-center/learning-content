...
`--package` (optional)::
  Package path to the application class.

`--baseTheme` (optional)::
  Base theme of the application: either `lumo` or `material`.
...
`src/main/java/<package>/HelloButton.java`::
  The server-side counterpart for the custom JavaScript/Polymer `hello-button.js` example component.

`src/main/java/<package>/MyProjectView.java`::
  The application view class for the root route, built from components.

`src/main/javascript/hello-button.js`::
  An example Polymer JavaScript component used by the *HelloButton* server-side component.

`src/main/stylesheets/theme.css`::
