...
The Java ecosystem has lots of excellent libraries, many really well-designed standards and often even multiple competing implementations for them. A link:/consuming-rest-services-from-java-applications[previous version of this blog post] highlighted https://jax-rs-spec.java.net/[JAX-RS], for example. Nowadays most Java web apps are based on Spring, so that is what we will use as an example here.
