...
Embedding everything under one Jar file can be useful in many cases, but sometimes we need to optimize and remove unneeded dependencies that might make the final file big. In this tutorial I've provided the minimal needed dependencies needed to get the application to work, so feel free to remove from `pom.xml` file any dependency or profile that you don't think you will need in the application. Here is a demo light version of Vaadin Flow as explained in this tutorial.