...
. In `CustomerForm.java`, we need a public API for the `MainView` to define which `Customer` to edit. We will also use `Binder` to make two-way databinding between the properties of the `Customer` and the UI fields in the form. 
...
* A `Binder` is an object that synchronize the properties from domain objects to the UI fields. The binder takes care of reading values from the business object, and converts user data from the format expected by the field to that expected by the business object. 
...
. Let's also improve the user experience by hidind the form when it is not bound to a domain object and focusing the first name text field when a user is selected. Amend the `setCustomer` method to:
...
. To initialize form state without an active customer, call `setCustomer(null)` at the end of the constructor.  
