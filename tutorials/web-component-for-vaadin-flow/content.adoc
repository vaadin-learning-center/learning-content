...
There is a https://github.com/vaadin/flow/issues/4909[bug] related to getting values from computed properties.

Similarly in `payment-methods`, `data` property is defined with a default value as a function and can be treated as a read-only value. Exectuion of a function is done using `callFunction()` and will be explained in a separete section below.
...
There is a https://github.com/vaadin/flow/issues/1724[bug] about implementing a return value for those APIs.
...
The next step is to add all those declared elementes as children to the `paymentRequest` element:
...
We also need to only add the `paymentReuest` component to the UI, nothing else.
...
A good idea to publish your work to the https://vaadin.com/director[Vaadin Directory] to get a feedback from the community and potential contributions from other users. Accessing the https://vaadin.com/directory/my-components?uploadNewComponent=[publish new compoenent] page gives pretty much all the information needed to proceed. We have done the first three steps already, and we are few maven commands away from having the add-on ready and published.

image::upload-addon.png[Upload Addon]

Check out the https://vaadin.com/directory/component/payment-request[add-on] created through out this tutorial and the https://github.com/amahdy/payment-request-flow[final source code].